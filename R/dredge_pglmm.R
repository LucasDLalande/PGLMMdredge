...
dredge_pglmm <- function(formulaRE, fixed, data,
  ...
  mod.RE <- pglmm(formula.RE, data, family,
                  cov_ranef = cov_ranef,
                  REML=F, verbose=F, s2.init=.1) # runs RE model
  ...
  for (i in 1:length(fixed)) {
    for (j in 1:length(comb.fix[[i]])) {
      ...
      form.fix <- c(form.fix, assign(form, str_replace(formulaRE, "1", l) |> as.formula())) # includes fixed effects within the null formula provided in the input

      mod.fix <- pglmm(eval(parse(text = form.fix)), data, family,
                       cov_ranef = cov_ranef,
                       REML=F, verbose=F, s2.init=.1) # runs fixed-effect models
  ...
  all_fixed_effects <- unique(unlist(lapply(model_list, function(mod) rownames(fixef(mod)))))   # gets the unique fixed effects from all models
  ...
  for (i in seq_along(model_list)) {
    model_fixed_effects <- rownames(fixef(model_list[[i]]))
    model_values <- as.data.frame(t(fixef(model_list[[i]])[,1]))
    model_se <- as.data.frame(t(fixef(model_list[[i]])[,2]))
